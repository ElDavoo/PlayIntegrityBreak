...
object PrefManager {
    ...
    private val pref = pibApp.getSharedPreferences("settings", MODE_PRIVATE)
    ...
    var hideIcon: Boolean
        ...
        set(value) {
            val enabled = findEnabledAppComponent(pibApp)
            if (value && enabled != null) {
                pibApp.packageManager.setComponentEnabledSetting(
                    enabled,
                ...
                pibApp.packageManager.setComponentEnabledSetting(
                    ComponentName(pibApp, AppConstants.COMPONENT_NAME_DEFAULT),
    ...
    var telemetryConsentPromptShown: Boolean
        get() = pref.getBoolean(PREF_TELEMETRY_CONSENT_PROMPT_SHOWN, false)
        set(value) = pref.edit { putBoolean(PREF_TELEMETRY_CONSENT_PROMPT_SHOWN, value) }
