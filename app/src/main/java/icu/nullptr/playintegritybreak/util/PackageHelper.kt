...
import icu.nullptr.playintegritybreak.ui.util.asComponentName
import icu.nullptr.playintegritybreak.ui.util.get
...
object PackageHelper {
    ...
    fun invalidateCache(
        ...
        pibApp.globalScope.launch {
            isRefreshing.emit(true)
            val cache = withContext(Dispatchers.IO) {
                val pm = pibApp.packageManager
                val um = pibApp.getSystemService(Context.USER_SERVICE) as UserManager
    ...
    fun loadAppIcon(packageName: String): Drawable = runBlocking {
        getCacheNoThrow()[packageName]?.icon ?:
            android.R.drawable.sym_def_app_icon.asDrawable(pibApp)
    }
    ...
    fun loadAppIconFromAppInfo(appInfo: ApplicationInfo): Drawable {
        return if (appInfo.packageName == BuildConfig.APPLICATION_ID) {
            val activityName = findEnabledAppComponent(pibApp)
            if (activityName == null) {
                R.mipmap.ic_launcher.asDrawable(pibApp)
            } else {
                pibApp.packageManager.getActivityIcon(activityName)
            }
        } else {
            try {
                pibApp.appIconLoader.loadIcon(appInfo).toDrawable(pibApp.resources)
            } catch (e: Throwable) {
                ServiceClient.log(Log.ERROR, TAG, e.stackTraceToString())

                try {
                    appInfo.loadIcon(pibApp.packageManager)
                } catch (x: Throwable) {
