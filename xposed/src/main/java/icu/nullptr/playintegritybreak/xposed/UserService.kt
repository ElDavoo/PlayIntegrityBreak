...
object UserService {
    ...
    private const val TAG = "PIB-UserService"

    private var appUid = 0

    private val uidObserver = object : UidObserverAdapter() {
        override fun onUidActive(uid: Int) {
            if (PIBService.instance == null) {
                logE(TAG, "PIBService instance is not available, maybe stopped")
                return
            }
            ...
            try {
                ...
                val extras = Bundle()
                extras.putBinder("binder", PIBService.instance)
    ...
    fun register(pms: IPackageManager, pmn: Any?) {
        logI(TAG, "Initialize PIBService - Version ${BuildConfig.APP_VERSION_NAME}")
        val service = PIBService(pms, pmn)
