...
object IntegrityServiceHook {
    ...
    private const val BLOCKED_CALLER_PACKAGE = "gr.nikolasspyr.integritycheck"
    private const val REWRITE_ERROR_CODE = -8
    private const val REWRITE_RETRIABLE = true

    // Keep rewrite behavior for parity with the PoC while the app-side policy UI is migrated.
    private const val ENABLE_RESPONSE_REWRITE = true
    ...
    private val blockedCallbackKeys = ConcurrentHashMap.newKeySet<String>()
    ...
    private fun hookMethod(method: Method) {
        XposedBridge.hookMethod(method, object : XC_MethodHook() {
            override fun beforeHookedMethod(param: MethodHookParam) {
                ...
                val blockedCaller =
                    BLOCKED_CALLER_PACKAGE == callerPkg || hasPackageStringArg(args, BLOCKED_CALLER_PACKAGE)
                val requestPayload = isIntegrityRequestPayload(args)
                val responsePayload = hasResponsePayload(args)
                val blockedByCallback = hasRememberedBlockedCallback(args)

                if (looksLikeRequest && callerPkg != "unknown") {
                    logI("Request from $callerPkg")
                }

                if (blockedCaller && requestPayload) {
                    rememberBlockedCallback(args)
                }

                val shouldRewrite =
                    ENABLE_RESPONSE_REWRITE && !requestPayload && responsePayload && (blockedCaller || blockedByCallback)

                if (shouldRewrite && rewriteResponseToBlockedError(args, REWRITE_ERROR_CODE, REWRITE_RETRIABLE)) {
                    val pkgForLog = if (callerPkg == "unknown") BLOCKED_CALLER_PACKAGE else callerPkg
                    logResult(pkgForLog, false, REWRITE_ERROR_CODE, REWRITE_RETRIABLE, "blocked-rewrite-pre")
            ...
            override fun afterHookedMethod(param: MethodHookParam) {
                ...
                if (outcome != null && callerPkg != "unknown") {
    ...
    private fun rememberBlockedCallback(args: Array<Any?>) {
        extractCallbackKey(args)?.let(blockedCallbackKeys::add)
    }

    private fun hasRememberedBlockedCallback(args: Array<Any?>): Boolean {
        val key = extractCallbackKey(args) ?: return false
        return blockedCallbackKeys.contains(key)
    }

    private fun clearRememberedBlockedCallback(args: Array<Any?>) {
        extractCallbackKey(args)?.let(blockedCallbackKeys::remove)
    }
